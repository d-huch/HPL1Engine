...
namespace hpl {
	...
	void cKeyboardSDL::Update()
	{
		...
		for (; it != mpLowLevelInputSDL->mlstEvents.end(); ++it)
		{
			...
			if (pEvent->type == SDL_KEYDOWN || pEvent->type == SDL_KEYUP)
			{
				...
				mvKeyArray[key] = (pEvent->type == SDL_KEYDOWN);
				...
				if (pEvent->type == SDL_KEYDOWN)
				{
					...
					mlstKeysPressed.push_back(cKeyPress(key, 0, mModifier));
				}
			}
			else if (pEvent->type == SDL_TEXTINPUT)
			{
				std::string inputText = pEvent->text.text;
				if (!inputText.empty())
				{
					char unicodeChar = inputText[0];
					mlstKeysPressed.push_back(cKeyPress(eKey_NONE, unicodeChar, mModifier));
				}
			}
		}
	}
	...
	eKey cKeyboardSDL::SDLToKey(int alKey)
	{
	    switch (alKey)
	    {
	        ...
	        case SDLK_RGUI: return eKey_RMETA; // Meta = GUI в SDL2
